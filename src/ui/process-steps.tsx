        ...
        <div className={cn(
          ...
          {steps.map((step, index) => (
            <div key={index} className={cn(
              "relative",
              orientation === 'horizontal' ? "flex-1 text-center lg:text-left" : "flex items-start"
            )}>
              {/* Step Number/Icon */}
              <div className={cn(
                "flex-shrink-0 flex items-center justify-center rounded-full text-white font-bold",
                colors.bg,
                orientation === 'horizontal' ? "w-12 h-12 mx-auto lg:mx-0 mb-4" : "w-10 h-10 mr-4 mt-1"
              )}>
              ...
              {index < steps.length - 1 && orientation === 'vertical' && (
                <div className={cn(
                  ...
                  "dark:" + colors.bg.replace('bg-', 'bg-').replace('-600', '-800')
                )} />
              )}
