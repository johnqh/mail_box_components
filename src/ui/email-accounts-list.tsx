...
const ChainPill: React.FC<{ type: 'primary' | 'ens' | 'sns'; addressType: 'evm' | 'solana' }> = ({ type, addressType }) => {
          ...
          <div className="flex items-center">
            <button
              ...
              className={cn(
                ...
                selectedAccount === group.primaryEmail.address && group.domainEmails.length === 0
                  ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 shadow-sm'
                  : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100'
              )}
              style={group.customColor ? { backgroundColor: group.customColor } : undefined}
            >
              <span className="truncate flex-1">{formatWalletAddress(group.walletAddress)}</span>
              <ChainPill type={group.primaryEmail.type} addressType={group.addressType} />
            </button>
              ...
              >
                <ChevronRightIcon 
                  className={cn(
                    ...
                    expandedWallets.includes(group.walletAddress) ? 'rotate-90' : 'rotate-0'
                  )} 
                />
              </button>
            )}
