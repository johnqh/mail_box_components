...
class PerformanceMonitor {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (nav.connection) {
        this.metrics.connection =
          nav.connection.effectiveType || nav.connection.type;
      }
  ...
  public trackRouteChange(
    route: string,
    pathOrTime: string | number,
    changeTime?: number
  ): void {
...
export const checkPerformanceBudget = (
  ...
  if (budget.LCP && metrics.LCP && metrics.LCP > budget.LCP) {
    violations.push(
      `LCP: ${metrics.LCP.toFixed(0)}ms (budget: ${budget.LCP}ms)`
    );
  }

  if (budget.INP && metrics.INP && metrics.INP > budget.INP) {
    violations.push(
      `INP: ${metrics.INP.toFixed(0)}ms (budget: ${budget.INP}ms)`
    );
  }
  ...
  if (budget.FCP && metrics.FCP && metrics.FCP > budget.FCP) {
    violations.push(
      `FCP: ${metrics.FCP.toFixed(0)}ms (budget: ${budget.FCP}ms)`
    );
  }

  if (budget.TTFB && metrics.TTFB && metrics.TTFB > budget.TTFB) {
    violations.push(
      `TTFB: ${metrics.TTFB.toFixed(0)}ms (budget: ${budget.TTFB}ms)`
    );
  }
...
export const exportPerformanceData = (metrics?: PerformanceMetrics): string => {
  const currentMetrics = metrics ||
    getPerformanceMonitor()?.getMetrics() || { timestamp: Date.now() };
  const exportData = {
