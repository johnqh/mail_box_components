...
export const FeatureGrid: React.FC<FeatureGridProps> = ({
  ...
  const renderFeature = (feature: Feature, index: number) => {
    const featureContent = (
      <div 
        className={cn(cardVariants({ variant: cardVariant, animation: cardAnimation }))}
      ...
      >
        ...
        {/* Icon */}
        <div 
          className={cn(iconVariants({ size: iconSize, style: iconStyle, position: iconPosition }))}
          style={getIconBackground(feature, index)}
        >
          <div className={cn(
            "flex items-center justify-center",
            iconStyle === 'colorful' ? "text-white" : "text-blue-600 dark:text-blue-400"
          )}>
            {feature.icon}
          </div>
        </div>
