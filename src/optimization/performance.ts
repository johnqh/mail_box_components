...
export const createLazyComponentFactory = <
...
export const performanceBudget = {
  ...
  check: (metrics: WebVitalsMetrics) => {
    ...
    if (metrics.FCP && metrics.FCP > performanceBudget.maxFCP) {
      violations.push(
        `FCP (${metrics.FCP}ms) exceeds budget (${performanceBudget.maxFCP}ms)`
      );
    }

    if (metrics.LCP && metrics.LCP > performanceBudget.maxLCP) {
      violations.push(
        `LCP (${metrics.LCP}ms) exceeds budget (${performanceBudget.maxLCP}ms)`
      );
    }

    if (metrics.FID && metrics.FID > performanceBudget.maxFID) {
      violations.push(
        `FID (${metrics.FID}ms) exceeds budget (${performanceBudget.maxFID}ms)`
      );
    }

    if (metrics.CLS && metrics.CLS > performanceBudget.maxCLS) {
      violations.push(
        `CLS (${metrics.CLS}) exceeds budget (${performanceBudget.maxCLS})`
      );
    }
