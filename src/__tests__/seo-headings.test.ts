...
describe('SEO Headings Utils', () => {
  ...
  describe('validateHeadingStructure function', () => {
    ...
    it('should detect multiple H1s', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Multiple H1 tags found (2) - use only one H1 per page'
      );
    });
    ...
    it('should detect heading level jumps', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Heading level jump from H1 to H4 - avoid skipping levels'
      );
    });
    ...
    it('should detect too short headings', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Heading 2 is too short - use descriptive text'
      );
    });

    it('should suggest shortening long headings', () => {
      const longText =
        'This is an extremely long heading that exceeds the recommended character limit for SEO optimization';
      const headings: HeadingConfig[] = [
        { level: 1, text: 'Title' },
  ...
  describe('WEB3_EMAIL_HEADINGS constants', () => {
    ...
    it('should have correct structure for predefined headings', () => {
      ...
      expect(WEB3_EMAIL_HEADINGS.emailBenefits.text).toBe(
        'Why Choose Web3 Email?'
      );

      expect(WEB3_EMAIL_HEADINGS.setupStep1.text).toBe(
        'Step 1: Connect Your Wallet'
      );
      expect(WEB3_EMAIL_HEADINGS.setupStep2.text).toBe(
        'Step 2: Verify Your Identity'
      );
      expect(WEB3_EMAIL_HEADINGS.setupStep3.text).toBe(
